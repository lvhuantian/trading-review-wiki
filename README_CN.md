...
- **Wiki 整理脚本** — 一键将 LLM 放错位置的根目录文件批量移动到正确的分类目录。详见 [`scripts/reorganize-wiki.cjs`](scripts/reorganize-wiki.cjs)
...
- **Wiki 整理脚本** —— 批量修复被 LLM 错误放到 wiki 根目录的文件，自动按 frontmatter type 归类到正确目录（[`scripts/reorganize-wiki.cjs`](scripts/reorganize-wiki.cjs)）
